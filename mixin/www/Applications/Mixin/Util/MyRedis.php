...
class MyRedis
{
     ...
     */
    public static function getInstance($config_name, $attr = array())
    {
        ...
        if (!isset(\Config\Redis::$$config_name)) {
            \Workerman\Worker::log("\"\\Config\\Redis::$config_name not set\n\"");
            $config = array(
                'host'    => '127.0.0.1',
                'port'    => 6379,
                'auth'    => '',
     ...
     * @param array $keyList
     */
    public function sUnionByStore($output="", $keyList=array())
    {
        if (is_array($keyList)){
            $output = $this->setKey($output);
            foreach ($keyList as $key){
                $key = $this->setKey($key);
                $this->redis->sUnionStore($output, $output, $key);
            }
        }
    }
